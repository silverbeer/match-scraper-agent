#!/bin/bash
#
# Backfill U16 HG Northeast matches from fall 2025 + spring 2026 into Missing Table.
#
# Usage:
#   ./scripts/backfill-u16-northeast.sh                          # Show plan (dry run)
#   ./scripts/backfill-u16-northeast.sh --run                    # Run week 1
#   ./scripts/backfill-u16-northeast.sh --run --week 3           # Run specific week
#   ./scripts/backfill-u16-northeast.sh --run --season fall      # Run all fall weeks
#   ./scripts/backfill-u16-northeast.sh --run --season spring    # Run all spring weeks
#   ./scripts/backfill-u16-northeast.sh --status                 # Check job status
#   ./scripts/backfill-u16-northeast.sh --cleanup                # Delete completed backfill jobs
#
...
if [[ "$ACTION" == "status" ]]; then
  ...
  kubectl get jobs -n "$NAMESPACE" -l task=backfill,division=northeast,age-group=u16 --sort-by=.metadata.creationTimestamp
  exit 0
fi

if [[ "$ACTION" == "cleanup" ]]; then
  echo "Deleting completed U16 Northeast backfill jobs..."
  kubectl delete jobs -n "$NAMESPACE" -l task=backfill,division=northeast,age-group=u16 --field-selector status.successful=1
  exit 0
fi

if [[ "$ACTION" == "plan" ]]; then
  echo "U16 HG Northeast Backfill Plan"
  echo "========================================="
  echo ""
...
run_week() {
  ...
  local job_name="backfill-u16-northeast-${season}-week${week_padded}"
...
run_season() {
  ...
  echo "Running ALL backfill weeks for U16 HG Northeast (${season})..."
...
if [[ -n "$WEEK_NUM" ]]; then
  ...
  echo "Running backfill ${SEASON_FILTER} week $WEEK_NUM for U16 HG Northeast..."
...
echo "View logs:    kubectl logs -n $NAMESPACE -l task=backfill,age-group=u16 --tail=50 -f"
